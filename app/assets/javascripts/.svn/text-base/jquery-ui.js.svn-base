 ...
 * Copyright (c) 2012 AUTHORS.txt; Licensed MIT, GPL */ (function (a, b) {
  ...
  a.widget("ui.autocomplete", {
    ...
    _create: function () {
      ...
      this.isMultiLine = this.element.is("textarea"), this.element.addClass("ui-autocomplete-input").attr("autocomplete", "off").attr({
        ...
        case e.DOWN:
          ...
          break;
        case e.ENTER:
          if($("#autocomplete_type").val()=='Need'){
            if($("#need_name_auto").val()){
                window.location.href = 'http://'+window.location.host+'/'+ $("#current_user").val()+'needs/' + $("#need_id").val();
            }
          }
          if($("#autocomplete_type").val()=='Category'){
            window.location.href = 'http://'+window.location.host+'/'+ $("#current_user").val()+'categories/' + $("#need_id").val();
          }
          if($("#autocomplete_type").val()=='Need_new'){
            if($("#need_name").val()){
              window.location.href = 'http://'+window.location.host+'/'+ $("#current_user").val()+'needs/' + $("#need_id").val();
            }
          }
    ...
    _renderMenu: function (b, c) {
      var d = this;
      if(c[0].type != 'Category'){
        a("<div></div>").text("Needs").css({ 'color': '#1874CD', 'font-size': '100%' }).appendTo(b)}
      i = 0
      a.each(c, function (a, c) {
        if(c.type == "Category"){i++}
        d._renderItem(b, c, i)
      })
    },
    _renderItem: function (b, c, flag) {
      if(c.type == 'Category' && flag == 1){a("<div>Categories</div>").css({ 'color': '#1874CD', 'font-size': '100%' }).appendTo(b)}
      return a("<li></li>").data("item.autocomplete", c).append((a("<a></a>").text(c.name))).appendTo(b)
      //return a("<li></li>").data("item.autocomplete", c).append("<a><table border='0'><tr><td>"+c.name+"</td><td style='color: #1874CD;'>"+c.needers+"</td></tr></table></a>").appendTo(b)
    },
    _move: function (a, b) {
